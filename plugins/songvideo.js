...
const { ytsearch, ytmp3, ytmp4 } = require('@cyber-rush/yt-dl.js'); 
...
async (conn, mek, m, { from, prefix, quoted, q, reply, waitForReply }) => {
    try {
        ...
        if (yt.results.rashu < 1) return reply("No results found!");
...
> *© 𝙿𝙾𝚆𝙴𝙰𝚁𝙳 𝙱𝚈 𝚀𝚄𝙴𝙴𝙽 𝚁𝙰𝚂𝙷𝚄 𝙼𝙳 ✾*`;
        ...
        const ytdl = await ytmp4(ytrejbakgksbu, quality);
...
> *© 𝙿𝙾𝚆𝙴𝙰𝚁𝙳 𝙱𝚈 𝚀𝚄𝙴𝙴𝙽 𝚁𝙰𝚂𝙷𝚄 𝙼𝙳 ✾*`
// SEND DETAILS
await conn.sendMessage(from,: yts.thumbnail || yts.image || '' }, caption: `${ytmsg}`}, { quoted: mek });

// SEND AUDIO TYPE
await conn.sendMessage(from,: ytdl.download.url }, mimetype: "audio/mpeg" }, { quoted: mek })
